...
function Compiler:new(pipeline)
    local isLuaU = pipeline and pipeline.LuaVersion == "LuaU";
    local compiler = {
        blocks = {};
        registers = {};
        activeBlock = nil;
...
function Compiler:compile(ast)
    ...
    local astItems;
    if self.isLuaU then
        -- LuaU: try _ENV first (Roblox), fallback to getfenv() (standalone Luau)
        -- In Roblox: _ENV is truthy, getfenv() never called → native codegen not de-optimized
        -- In standalone Luau: _ENV is nil, getfenv() called as fallback
        astItems = {
            Ast.OrExpression(Ast.VariableExpression(newGlobalScope, envVar), Ast.AndExpression(Ast.VariableExpression(newGlobalScope, getfenvVar), Ast.FunctionCallExpression(Ast.VariableExpression(newGlobalScope, getfenvVar), {})));
