...
function Pipeline:apply(code, filename)
	...
	if(self.Seed > 0) then
		...
		local success, seed = pcall(function()
			local seedStr = io.popen("openssl rand -hex 8"):read("*a"):gsub("%s+", "")
			--> Parse only first 8 hex chars (32 bits) to stay within integer precision
			local seedNum = 0;
			local len = math.min(#seedStr, 8);
			for i = 1, len do
			...
			if seedNum == 0 then seedNum = os.time() end
			return seedNum
		end)

		if success then
			math.randomseed(seed)
			logger:info(string.format("Random seed generated via OpenSSL: %.0f", seed));
		else
			logger:warn("OpenSSL is unavailable. Falling back to unix time.");
			local timeSeed = os.time();
			math.randomseed(timeSeed)
			logger:info(string.format("Random seed generated via os.time(): %d", timeSeed));
		end
	end
	...
	-- Add watermark as block comment before code
	if self._watermarkContent then
		code = "--[[ " .. self._watermarkContent .. " ]] " .. code;
	end

	-- Add directives for LuaU (plaintext - required by Luau compiler)
	if self.LuaVersion == "LuaU" then
		code = "--!native\n--!strict\n" .. code;
		logger:info("Added --!native, --!strict directives for LuaU");
	end
